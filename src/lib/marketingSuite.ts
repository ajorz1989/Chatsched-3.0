...
const LOCATION_HINTS = [
  ...
  "gqeberha", "bloemfontein", "western cape", "gauteng", "kwazulu", "eastern cape",
];
  ...
  const scored = publishers.map((p) => {
    ...
    if (budget != null && budget > 0) {
      if (p.price_per_post <= budget) {
        score += 10;
        reasons.push(`Price R${p.price_per_post} fits your budget`);
      } else if (p.price_per_post <= budget * 1.25) {
        score += 4;
        reasons.push(`Slightly above budget (R${p.price_per_post})`);
      } else {
        score -= 5;
      }
    }
