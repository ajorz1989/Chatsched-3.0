...
import type { ChannelDefinition, AnalyticsMetric, PricingModel } from "../lib/channelTypes";
...
function PricingCard({ model }: { model: PricingModel }) {
  return (
    <div className="border-[3px] border-billboard-ink rounded p-4">
      <div className="font-bold text-sm mb-1">{model.label}</div>
      <div className="text-billboard-inkSoft text-sm mb-3">{model.description}</div>
      <div className="font-mono text-xs text-billboard-inkSoft">
        From R{model.minPrice.toLocaleString()}
      </div>
    </div>
  );
}
...
function SharedSections({ ch }: { ch: ChannelDefinition }) {
  ...
  return (
    <>
      {/* Pricing */}
      <section>
        <h2 className="font-display text-xl mb-4">Pricing models</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {ch.pricingModels.map((m) => (
            <PricingCard key={m.unit + m.label} model={m} />
          ))}
        </div>
      </section>
...
export default function ChannelPage() {
  ...
  return (
    <>
      <Seo
        title={`${ch.name} — Micro Billboards`}
